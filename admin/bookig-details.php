...
if (strlen($_SESSION['alogin']) == 0) {
	...
	if (isset($_REQUEST['aeid'])) {
		...
		if($tenant == 'tenant')
		{
			echo "<script type='text/javascript'> document.location = 'confirmed-bookings.php?from=tenant'; </script>";
												...
												if ($query->rowCount() > 0) {
													foreach ($results as $result) { ?>
														...
														<tr>
															<th>Vehicle Name</th>
															<td><a
																	href="edit-vehicle.php?id=<?php echo htmlentities($result->vid); ?>"><?php echo htmlentities($result->BrandName); ?> , <?php echo htmlentities($result->VehiclesTitle); ?></td>
															<th>Booking Date</th>
															<td>
